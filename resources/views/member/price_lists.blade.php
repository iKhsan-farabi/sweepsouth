...
@section('main-content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Pilihan Layanan</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
                            ...
                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link active" id="kiloan-tab" data-toggle="tab" href="#kiloan"
                                        role="tab" aria-controls="kiloan" aria-selected="true">Layanan Lain</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link" id="satuan-tab" data-toggle="tab" href="#satuan" role="tab"
                                        aria-controls="satuan" aria-selected="false">Cleaning</a>
                                </li>
                            </ul>
                                    ...
                                    <div class="row">
                                        @foreach($kiloan as $k)
                                          <div class="col-md-4">
                                            <div class="card">
                                              <div class="card-body">
                                                <h5 class="card-title"><b>{{ $k->item->name }}</b></h5>
                                                <p class="card-text">{{ $k->service->name }}</p>
                                                <a href="#" class="btn btn-primary">{{ $k->getFormattedPrice() }}</a>
                                              </div>
                                            </div>
                                          </div>
                                        @endforeach
                                    ...
                                    <div class="row">
                                        @foreach($satuan as $s)
                                          <div class="col-md-4">
                                            <div class="card">
                                              <div class="card-body">
                                                <h5 class="card-title"><b>{{ $s->item->name }}</b></h5>
                                                <p class="card-text">{{ $s->service->name }}</p>
                                                <a href="#" class="btn btn-primary">{{ $s->getFormattedPrice() }}</a>
                                              </div>
                                            </div>
                                          </div>
                                        @endforeach
